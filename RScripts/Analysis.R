    ...
    for (TimepointsSampled_index in 1:length(TimepointsSampled)) {
      ...
      for (NExpRep in 1:max(NsExpRep)){
        for (Direction in Directions){
          ...
          full_output_temp <- cbind(full_output_temp, s_ABC=NA, p_App=NA)
          ...
          for (computational_method in ComputationalMethods){
            if (computational_method == "WFABC"){
              ...
              post_s <- read.table(paste0("ABCOutput", out_name, "_NGen5.txt")) ## Might need to change this here
              ...
              mcmc <- fread(paste0("AppOutput", out_name,"_NGen5.txt"))
              mcmc<-data.frame(mcmc)
              mcmc <- mcmc[-c(1:10000),]
              #s_App <- apply(mcmc[,-c(1:4)], 2, mean) ## Attention, might need to exclude first four columns depending on the ApproxWF setting!
              get_p <- function(x){
                return(length(which(x>0))/length(x))
              }
              p_App <-  apply(mcmc[,-c(1:4)], 2, get_p)
              if (Direction == "Plus"){
                #full_output_temp$s_App <- s_App ## this is the proportion of posterior distribution larger than 0
                full_output_temp$p_App <- p_App
              } else {
                #full_output_temp$s_App <- -s_App
                full_output_temp$p_App <- 1-p_App
              }
            }
          }
          ...
          if (!exists("full_output")){
            ...
            if("WFABC" %in% ComputationalMethods){
              full_output_temp[,c(8, 16)][is.na(full_output_temp[,c(8, 16)])] <- 0 ## turn all NAs in s_ABC into 0
            ...
            if("ApproxWF" %in% ComputationalMethods){
              full_output_temp[,c(9, 17)][is.na(full_output_temp[,c(9, 17)])] <- 0.5 ## turn all NAs in p_App into 0.5
              full_output[,9]<-((weight-1)*full_output_temp[,9]+full_output_temp[,17])/weight # Calculate average p-value
            }
            colnames(full_output) <-c("ID_perm", "type",  "position", "es", "dominance", "origin", "D", "s_ABC", "p_App")
          }
          if (NExpRep %in% NsExpRep){
            ...
            for (computational_method in ComputationalMethods){
              if (out_name=="AllTimepoints" & computational_method == "D" & NGen == 5){ # Make sure that ROC for D isn't calculated twice, once with all time points and once with two
                ...
                if (computational_method == "D"){
                  ...
                  abs_p_App <- abs(full_output$p_App-0.5)*2
                }
                ...
                for (window_size in WindowSizes){ 
                  ...
                  Power_percentage_QTL_detected_unweighted <- rep(0, NROCPoints)
                  ...
                  for (i in 1:NROCPoints){
                    if (computational_method == "D"){
                      ...
                      #cutoff <- max(abs_s_ABC)*(i-1)/NROCPoints
                      cutoff <- 1*(i-1)/NROCPoints
                      detected_pos <- sampled_SNP_pos[which(abs_s_ABC>=cutoff)]
                    } else if (computational_method == "ApproxWF"){
                      #cutoff <- max(abs_s_App)*(i-1)/NROCPoints
                      #detected_pos <- sampled_SNP_pos[which(abs_s_App>=cutoff)]
                      cutoff <- max(abs_p_App)*(i-1)/NROCPoints
                      detected_pos <- sampled_SNP_pos[which(abs_p_App>=cutoff)]
                    }
                    ...
                    if (length(detected_QTL_ID_perm)==0){
                      ...
                      Power_percentage_QTL_detected_unweighted[i] <- 0
                    } else {
                      ...
                      Power_percentage_QTL_detected_unweighted[i] <-length(detected_QTL_pos)/length(gen_1_QTL_pos)
                    }
                    ## False positive
                  ...
                  ROC_temp<-data.frame(Power_gen_1,Power_percentage_QTL_detected, False_positive, Window_size, computational_method, out_name, Power_percentage_QTL_detected_unweighted)
            ...
            if (exists("ROC") & Direction==Directions[length(Directions)]){ 
              write.table(ROC, paste0("ROC_", out_name, "_NGen5.txt"), sep=",", col.names=T, row.names = F, quote = F)
