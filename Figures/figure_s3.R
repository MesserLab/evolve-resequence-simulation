...
if (Type %in% c("QTL_architecture", "Experimental_design")){ # define filenames, plot title, and plot labels in the begining of the following loop 
  for (i in c(11)){
    ...
    for (Direction in Directions) {
      ...
      p3 <- ggplot(ROC_QTL_archetecture_pooled, aes(False_positive_rate,Proportion_of_QTL_detected_unweighted, color = factor(color, labels = labels))) +
        ...
        coord_cartesian(xlim = c(0,0.1)) +
        xlab("False positive rate") +
        ylab("Proportion of QTL detected") +
        theme_bw() +
        theme(legend.position="none") +
        theme(axis.title = element_text(size = 30)) +
        theme(axis.text=element_text(size=30)) +
  ...
  if (Type == "Replication"){
    NsExpRep <- paste0(Direction, NsExpRep)
  } else{
    NsExpRep <- paste0(Directions, NExpRep)
  }
  ...
  p1 <- ggplot(ROC_final, aes(False_positive_rate,Proportion_of_genetic_variance_in_gen_1, color = factor(color, labels = NsExpRep))) + 
    ...
    geom_line(size = 2, alpha = 0.8, linetype="dotted") + 
    #geom_point(data=ROC_final[which(ROC_final[,1]>0 | ROC_final[,3]>0),], aes(False_positive_rate, Proportion_of_genetic_variance_in_gen_1, color = factor(color, labels = NsExpRep)), size = 3, alpha = 0.8) + 
    geom_line(data=ROC_final[which(ROC_final[,1]>0 | ROC_final[,3]>0),], aes(False_positive_rate, Proportion_of_genetic_variance_in_gen_1, color = factor(color, labels = NsExpRep)), size = 2, alpha = 1) + 
  ...
  p3 <- ggplot(ROC_final, aes(False_positive_rate,Proportion_of_QTL_detected_weighted, color = factor(color, labels = NsExpRep))) + 
    #geom_point(size = 3, alpha = 0.8, shape=1) + 
    geom_line(size = 2, alpha = 0.8, linetype="dotted") + 
    #geom_point(data=ROC_final[which(ROC_final[,2]>0 | ROC_final[,3]>0),], aes(False_positive_rate, Proportion_of_QTL_detected_weighted, color = factor(color, labels = NsExpRep)), size = 3, alpha = 0.8) + 
    geom_line(data=ROC_final[which(ROC_final[,2]>0 | ROC_final[,3]>0),], aes(False_positive_rate, Proportion_of_QTL_detected_weighted, color = factor(color, labels = NsExpRep)), size = 2, alpha = 1) + 
    scale_x_continuous(limits = c(0,1), breaks = seq(0,1,0.2)) + 
    xlab("False positive rate") +
    ylab("Proportion of QTL detected weighted by effect size") +
    guides(color=guide_legend(title="")) +
  ...
  for (Direction in Directions){
    ...
    png(paste0("../../FiguresForPaper/Figure_S3.png"), width = 800, height = 750, units = "px", pointsize = 20)
    print(p4)
    dev.off()
