...
if (Type %in% c("QTL_architecture", "Experimental_design")){ # define filenames, plot title, and plot labels in the begining of the following loop 
  for (i in c(1)){
    ...
    labels <- list(c("~14000 SNPs", "~1400 SNPs")
...
library(cowplot)
figure_s1 <- plot_grid(p6, p4, labels=c("A", "B"), nrow = 1, label_size=30)
png(paste0("~/evolve-resequence-simulation/Figures/figure_s1.png"), width = 800*2, height = 750, units = "px", pointsize = 20)
print(figure_s1)
dev.off()
